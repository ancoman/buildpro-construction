  ...
  whoPeople: [
    {
      ...
      bio: "Over 10 years leading construction projects from planning to delivery. Specializes in complex renovations and commercial builds.",
    ...
    },
    {
      ...
      role: "Lead Architect",
      bio: "Award-winning architect with a passion for sustainable design. Brings creative vision and technical precision to every project.",
      cover: "https://images.unsplash.com/photo-1494790108755-2616b612b9e5?q=80&w=400&auto=format&fit=crop",
      media: [],
      linkedin: "",
      github: "",
      email: ""
    },
    {
      ...
      bio: "Hands-on field experience across residential and industrial sites. Ensures quality standards and safety protocols are met daily.",
      cover: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=400&auto=format&fit=crop",
      media: [],
      linkedin: "",
      github: "",
      email: ""
...
function setupContactScreen() {
  ...
  } else if (location.hash === "#team-members") {
    showTeamUI();
    history.replaceState({ team: true }, "", "#team-members");
  }
...
function setupWhoTeamScreen() {
  document.getElementById("team-preview-open")?.addEventListener("click", (e) => {
    e.preventDefault();
    openTeamScreen();
  });

  document.getElementById("team-preview-image")?.addEventListener("click", () => {
    openTeamScreen();
  });

  document.getElementById("add-person-btn-main")?.addEventListener("click", () => addPerson());
}
...
function openTeamScreen() {
  ...
  showTeamUI();
  if (location.hash !== "#team-members") {
    history.pushState({ team: true }, "", "#team-members");
  }
}
...
function renderTeamPreview() {
  ...
  const titleEl = document.getElementById("team-preview-who-title");
  if (titleEl) titleEl.textContent = whoTitle;

  const imgEl = document.getElementById("team-preview-image");
  if (imgEl) { imgEl.src = whoImg; imgEl.alt = whoTitle; }

  const textEl = document.getElementById("team-preview-text");
  if (textEl) textEl.innerHTML = formatAboutText(whoText);

  if (isAdmin()) {
    const tIn = document.getElementById("team-preview-title-input");
    if (tIn) tIn.value = c.whoTitle ?? d.whoTitle ?? "";
    const txIn = document.getElementById("team-preview-text-input");
    if (txIn) txIn.value = c.whoText ?? d.whoText ?? "";
    const urlIn = document.getElementById("team-preview-photo-url");
    if (urlIn) urlIn.value = c.whoSectionImage ?? d.whoSectionImage ?? "";
    setImageEditorPreview("team-preview-photo-preview", whoImg);
  }
}

async function saveTeamPreviewSection() {
  if (!isAdmin()) return;
  const d = defaultData.content;
  const title = document.getElementById("team-preview-title-input").value.trim();
  siteData.content.whoTitle = title || siteData.content.whoTitle || d.whoTitle;
  siteData.content.whoText = document.getElementById("team-preview-text-input").value.trim();
  const img = await resolveUrlOrFile("team-preview-photo-url", "team-preview-photo-file");
...
function renderTeamScreenList() {
  ...
  siteData.whoPeople.forEach((person) => {
    ...
    avatarWrap.className = "team-card-avatar-wrap";
    if (person.cover && person.cover !== PLACEHOLDER) {
      const img = document.createElement("img");
      img.src = person.cover;
      img.alt = person.name;
      img.className = "team-card-avatar";
      avatarWrap.appendChild(img);
    } else {
      const initials = person.name.split(" ").filter(Boolean).map((w) => w[0]).join("").slice(0, 2).toUpperCase();
      const div = document.createElement("div");
      div.className = "team-card-avatar-initials";
    ...
    if (person.bio) {
      ...
      bioEl.className = "team-card-bio";
      bioEl.textContent = person.bio;
      card.appendChild(bioEl);
    }

    const hasSocials = person.linkedin || person.github || person.email;
    if (hasSocials) {
      const socials = document.createElement("div");
      socials.className = "team-card-socials";
      if (person.linkedin) {
        const a = document.createElement("a");
        a.href = person.linkedin; a.target = "_blank"; a.rel = "noopener noreferrer";
        a.className = "team-card-social-link"; a.textContent = "LinkedIn";
        a.addEventListener("click", (e) => e.stopPropagation());
        socials.appendChild(a);
      }
      if (person.github) {
        const a = document.createElement("a");
        a.href = person.github; a.target = "_blank"; a.rel = "noopener noreferrer";
        a.className = "team-card-social-link"; a.textContent = "GitHub";
        a.addEventListener("click", (e) => e.stopPropagation());
        socials.appendChild(a);
      }
      if (person.email) {
        ...
        a.className = "team-card-social-link"; a.textContent = "Email";
        a.addEventListener("click", (e) => e.stopPropagation());
        socials.appendChild(a);
      }
    ...
    const adminHtml = adminCardActionsHtml("person", person.id);
    if (adminHtml) {
      const adminWrap = document.createElement("div");
      adminWrap.innerHTML = adminHtml;
      card.appendChild(adminWrap.firstElementChild);
    }

    card.addEventListener("click", (e) => {
      if (e.target.closest(".card-admin-bar") || e.target.closest(".team-card-social-link")) return;
...
function bindMainAdmin() {
  ...
  document.getElementById("save-footer-program").addEventListener("click", saveFooterProgram);
  document.getElementById("save-footer-services").addEventListener("click", saveFooterServices);
  document.getElementById("save-footer-contact").addEventListener("click", saveFooterContact);
  document.getElementById("save-team-preview").addEventListener("click", saveTeamPreviewSection);
...
function saveFooterProgram() {
  if (!isAdmin()) return;
  siteData.content.contactProgramText = document.getElementById("footer-program-text-input").value.trim();
  siteData.content.footerLegalTitle = document.getElementById("footer-legal-title-input").value.trim();
  siteData.content.footerLegalText = document.getElementById("footer-legal-text-input").value.trim();
  closeAllAdminPanels();
  persistAndRender();
  toast("Program column saved.");
}

function saveFooterServices() {
  if (!isAdmin()) return;
  siteData.content.servicesTitle = document.getElementById("footer-services-title-input").value.trim() || siteData.content.servicesTitle;
  closeAllAdminPanels();
  persistAndRender();
  toast("Services column saved.");
}

function saveFooterContact() {
  if (!isAdmin()) return;
  siteData.content.contactFooterAddress = document.getElementById("footer-contact-address-input").value.trim();
  siteData.content.contactFooterPhone = document.getElementById("footer-contact-phone-input").value.trim();
  siteData.content.contactFooterEmail = document.getElementById("footer-contact-email-input").value.trim();
  closeAllAdminPanels();
  persistAndRender();
  toast("Contact column saved.");
}
...
function renderSiteFooter() {
  ...
  if (isAdmin()) {
    const progIn = document.getElementById("footer-program-text-input");
    if (progIn) progIn.value = c.contactProgramText ?? d.contactProgramText ?? "";
    const legalTitleIn = document.getElementById("footer-legal-title-input");
    if (legalTitleIn) legalTitleIn.value = c.footerLegalTitle ?? d.footerLegalTitle ?? "";
    const legalTextIn = document.getElementById("footer-legal-text-input");
    if (legalTextIn) legalTextIn.value = c.footerLegalText ?? d.footerLegalText ?? "";
    const svcTitleIn = document.getElementById("footer-services-title-input");
    if (svcTitleIn) svcTitleIn.value = c.servicesTitle ?? d.servicesTitle ?? "";
    const addrIn = document.getElementById("footer-contact-address-input");
    if (addrIn) addrIn.value = c.contactFooterAddress ?? d.contactFooterAddress ?? "";
    const phoneIn = document.getElementById("footer-contact-phone-input");
    if (phoneIn) phoneIn.value = c.contactFooterPhone ?? d.contactFooterPhone ?? "";
    const emailIn = document.getElementById("footer-contact-email-input");
    if (emailIn) emailIn.value = c.contactFooterEmail ?? d.contactFooterEmail ?? "";
  }
...
function fillPersonAdminForm(person) {
  ...
  document.getElementById("person-linkedin-input").value = person.linkedin || "";
  document.getElementById("person-github-input").value = person.github || "";
...
async function savePersonItem() {
  ...
  person.role = document.getElementById("person-role-input").value.trim();
  person.bio = document.getElementById("person-bio-input").value.trim();
  person.linkedin = document.getElementById("person-linkedin-input").value.trim();
  person.github = document.getElementById("person-github-input").value.trim();
  person.email = document.getElementById("person-email-input").value.trim();
...
function setupInfoDrawer() {
  const toggleBtn = document.getElementById("nav-drawer-open");
  ...
  toggleBtn.addEventListener("click", () => {
    infoDrawerOpen ? closeInfoDrawer() : openInfoDrawer();
  });
  closeBtn.addEventListener("click", closeInfoDrawer);
  overlay.addEventListener("click", closeInfoDrawer);

  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && infoDrawerOpen) closeInfoDrawer();
  });
}
...
function openInfoDrawer() {
  ...
  const overlay = document.getElementById("info-drawer-overlay");
  const btn     = document.getElementById("nav-drawer-open");
  if (!drawer) return;
...
function closeInfoDrawer() {
  ...
  drawer.addEventListener("transitionend", () => {
    if (!infoDrawerOpen) drawer.hidden = true;
  }, { once: true });
  infoDrawerOpen = false;
}

function populateInfoDrawer() {
  const c = siteData.content;
  const d = defaultData.content;
  const details = getFooterContactDetails(c, d);
  ...
  if (addressEl) addressEl.innerHTML = details.addressHtml;

  if (emailEl) {
    emailEl.textContent = details.email || "";
    emailEl.href = details.email ? `mailto:${details.email}` : "#";
    const row = document.getElementById("drawer-email-row");
    if (row) row.style.display = details.email ? "" : "none";
  }

  if (phoneEl) {
    phoneEl.textContent = details.phone || "";
    phoneEl.href = details.phone ? `tel:${details.phone.replace(/\s/g, "")}` : "#";
    const row = document.getElementById("drawer-phone-row");
    if (row) row.style.display = details.phone ? "" : "none";
  }

  const introEl = document.getElementById("drawer-intro-text");
  if (introEl) {
    const intro = (c.contactDrawerIntro || d.contactDrawerIntro || "").trim();
